...
void Scrub::readfromCSV(std::ifstream & ip, long pos, long pos_next){
	if (ip.fail())  {
		std::cout << "File not found" << std::endl; int theFix;
		std::cout << "Enter a character to exit" << std::endl;
		std::cin >> theFix;
	...
	std::string newline;
	std::getline(ip, newline); //remove the firstline
	while (ip.tellg() <= pos_next && std::getline(ip, newline) && ip.good()){
		if (newline.empty())
		{
			continue;
		}
